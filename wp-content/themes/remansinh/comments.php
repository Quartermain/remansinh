...
<div class="space"></div>
    <section id="comments" class="well well-sm">
        ...
        <?php if ( post_password_required() ) : ?>
            <p class="nopassword">
                <?php _e( 'This post is password protected. Enter the password to view any comments.', 'elano' ); ?>
            </p>
        <?php return; endif; ?>
        ...
        if ($ncom>0) :
            ...
            _e('Comments','elano');
            echo '&nbsp;(';
            if ($ncom==1) _e('1', 'elano'); else echo sprintf (__('%s','elano'), $ncom);
            echo ')</h3>';
    ...
    'comment_notes_after' => '<button class="btn-color btn-color-1 btn-color-1d" type="submit" id="submit"><span>'.__('Post Comment').'</span></button>' ,
            ...
            'logged_in_as' => '<p class="logged-in-as">' . sprintf( __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>','elano'), get_edit_user_link(), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink($post->ID) ) ) ) . '</p>',
